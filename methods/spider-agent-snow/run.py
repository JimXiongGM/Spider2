...
def test(args: argparse.Namespace, test_all_meta: dict = None) -> None:
    ...
    valid_ids = []
    ## load task configs
    assert os.path.exists(args.test_path) and args.test_path.endswith(
        ".jsonl"
    ), f"Invalid test_path, must be a valid jsonl file: {args.test_path}"
    ...
    for task_config in task_configs:
        ...
        task_config["config"] = [
            {
                ...
                "parameters": {"dirs": [os.path.join(source_data_dir, task_config["instance_id"])]},
            }
        ]

        env = Spider_Agent_Env(
            env_config=env_config, task_config=task_config, cache_dir="./cache", mnt_dir=output_dir
        )
        ...
        # Delete sqlite files
        if task_type == "local":
            sqlite_files = glob.glob(os.path.join(output_dir, "*.sqlite")) + glob.glob(
                os.path.join(output_dir, "*.duckdb")
            )
